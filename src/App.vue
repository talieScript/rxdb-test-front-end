<template>
  <v-app>
    ...
    <v-content>
      ...
      />
       <p v-else class="text-center text--secondary">Select or create a submission.</p>
    </v-content>
  </v-app>
</template>
  ...
  async mounted() {
    getDb().then(async (db) => {
      ...
      db.submission.$.subscribe(async event => {
        ...
        if(event.data.op === 'REMOVE' && event.data.doc === this.activeSubmission.id) {
